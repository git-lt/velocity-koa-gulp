...
K.extend(KSWFUpload, {
	init : function(options) {
		...
		self.bodyDiv = K('.ke-swfupload-body');
		...
		var settings = {
			...
			upload_success_handler : function(file, serverData) {
				...
				file.url = "https://qncdn.qiakr.com/"+data.key;
	...
	getUrlList : function() {
		var list = [];
		K('.ke-swfupload-body .ke-img').each(function() {
			var img = K(this);
		...
		self.plugin.multiImageDialog({
			clickFn : function (urlList) {
				...
				K.each(urlList, function(i, data) {
					var url = "https://qncdn.qiakr.com/"+data.key;
					if (self.afterUpload) {
						self.afterUpload.call(self, url, data, 'multiimage');
					}
					self.exec('insertimage', url, data.title, data.width, data.height, data.border, data.align);
				});
