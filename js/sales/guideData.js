...
var DO = function(){
	...
	this.pickers = {};
	this.getSalesDataBetweenUrl = '../admin/getSalesDataBetween.json';
	this.getSalesDataStatListUrl = '../admin/getSalesDataStatList.json';
	this.getStoreList = '../admin/getStoreList.json';
	this.getsalesList = '../admin/getSalesListOfStore.json';
}
...
DO.prototype = {
    ...
    loadData: function(sTime, eTime, sel,timeSpace,sTime2,param){
		...
		/*console.log(param.salesId);*/
    	$.ajax({
    		url: _this.getSalesDataBetweenUrl,
            type: "post",
            data: {
    	...
    	$.ajax({
            url: _this.getSalesDataStatListUrl,
            type: "post",
            data: {
            	token: getUrlParam("token"),
                startTime: sTime||param.sTimeB, 
                endTime : eTime||param.eTimeB,
                salesId : param.salesId||''
            },
            success: function(res){
            	...
            	$(".chartEvt").each(function(i){
            		...
            		if (timeSpace==undefined||timeSpace=="") {
            			$.each(res.result.statYmdVoList, function(j, item){
		                    ...
		                    switch(i+1){
		                    	...
		                    	case 2: val = item.customerSubscribeCount;break;
			...
			case 1: 
				return '<div class="toolbar">' +
				            '<div class="toolbar-inner">' +
				                ...
				                '<div class="right">' +
				                    '<a href="#" id="storeConfirm" class="link close-picker">下一步</a>' +
				                '</div>' +
				            '</div>' +
				        '</div>';
			case 2: 
				return '<div class="toolbar">' +
				            '<div class="toolbar-inner">' +
				                '<div class="left">' +
				                    '<a href="#" id="cancel" class="link toolbar-randomize-link">取消</a>' +
				                '</div>' +
				                '<div class="center">　请选择导购</div>' +
				                '<div class="right">' +
				                    '<a href="#" id="salesConfirm" class="link close-picker">确定</a>' +
				                '</div>' +
				            '</div>' +
				        '</div>';   
			}
    ...
    opensalesList:function(storeId){
    	var _this = this;
    	$.ajax({
    		url: _this.getsalesList,
    		type: 'post',
    		data: {
    			storeId: storeId
    		},
    			 ...
    			 if (res.status === '0'){
			    	var pickerCustomToolbar = myApp.picker({
					    ...
					    cols: [
					        {
					            ...
					            values: (function(){
					            	var salesArr=[];
					            	salesArr.push('所有导购');
					            	for (var i=0;i<=res.result.salesAdminVoList.length-1;i++) {
			    						salesArr.push(res.result.salesAdminVoList[i].name)
			    					}
			    					return salesArr;
					            })(),
					        }
				    	...
				    	$("#cancel").off().on("click", function(){
					 	...
					 	$('#salesConfirm').off().on('click',function(){
					 		var salesPicker = pickerCustomToolbar.cols[0].value;
					 		if(salesPicker!='所有导购'){
					 			for(var i=0;i<=res.result.salesAdminVoList.length-1;i++){
					 				if(res.result.salesAdminVoList[i].name==salesPicker){
					 					var salesId = res.result.salesAdminVoList[i].salesId;
					 					break;
					 				}
					 			}
					 			$("#param").attr("data-salesId", salesId);
					 			console.log(salesId)
					 		}
					 		var param = _this.getCondition();
					 		$("#mask").removeClass("mask");
					...
					pickerCustomToolbar.open();
    			 	...
    			 	return mobileAlert(res.errmsg ? res.errmsg : "获取导购列表失败！");
    			 }
    		}
    	})
    },
