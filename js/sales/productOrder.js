var DO = function(){
	...
	this.pickers = {};
	this.getProductOrder = '../admin/getProductOrderRank.json';
}
    ...
    loadData: function(sTime, eTime, sel){
    	...
    	$.ajax({
	        ...
	        success:function(res){
	            if (res.status === '0') {
	                var html = template('productOrderTpl', {list:res.result.productOrderRank});
	                $("#content").html(html);
	                var rankLength = res.result.productOrderRank.length;
	                if(rankLength===0){
	                	$(".noneData").text("暂无数据");
	                }
	            }
