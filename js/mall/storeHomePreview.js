...
define(['swiper', 'iscroll', 'qrcode'],function(a, iScroll){
	...
	PRO_LINK = 'getStockInfoForCustomer.htm?salesId='+PAGE_CONF.salesId;
	...
	(function($, window){
		...
		RenderShopConfig.prototype = {
			...
			_render:function(shopConf, callback){
	    	...
	    	// 模块排序
				...
				debounce(function(){
					...
					if(!PREVIEW){
						...
						page.attentionEv();
						// 商品分享storeId修正
						$('a[href*="shareStockInfoForCustomer"]').each(function(i,e){
			...
			_creatProList:function(proConfData){
				...
				if(proConfData.autoRecommend){ //自动推荐 异步请求数据
						...
						function prosessData(){
							...
							if(typeof curr !== 'undefined'){
								...
								.done(function(data, a, b){
							    	...
							    	debounce(function(){scrollMain && scrollMain.refresh();}, 300)();
									}else{
										dtd.resolve();
									}
									prosessData();
					...
					var pDataOrder = pData.proInfo.sort(function(a, b){a.orderNum-b.orderNum});
	...
	page = {
		init:function(){
			...
			if(!PREVIEW){
				...
				this.recordHistory();
				this.searchEv();			
				PAGE_CONF.salesId && (sessionStorage.salesId = PAGE_CONF.salesId);
			}
      ...
      localStorage.storeHistory3 = JSON.stringify(totalHistory);
		...
		showSiderbarEv:function(){ //边栏
			$('#categoryMenu').on('click', function() {
		        ...
		        clsOut: 'slideOutLeft',
		        width: '100%',
		        bodyStyle: 'position: fixed; z-index: 500; top: 0;right: 0;bottom: 0;left: 0; background-color:#fff; padding:0; overflow:hidden;',
