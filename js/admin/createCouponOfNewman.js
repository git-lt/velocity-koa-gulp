...
var _couponDialog,createPromotion = {
    init:function(){
        this.timeInit();
        this.couponBoxShow();
    ...
    limitStore:function(){
        $("select[name=promotionType]").on("change",function(){
            if($(this).val()=="6"){
                $("#limitStoreRow").show();
            }else{
                $("#limitStoreRow").hide();
                $("input[name=grantValue]").val("");
            }
        });
        if($("select[name=promotionType]").val()=="6"){
            $("#limitStoreRow").show();
        }
        $("#grantStoreLimitEdit").on("click",function(){
            $("#grantLimitStore").trigger("change");
        });
    },
                ...
                success:function(data){
                    if(data.status=="0"){
                        Util.alert("更新成功",function(){
                            location.href="activityCouponOfNewman.htm";
                        });
                    }else{
                        Util.alert(data.errmsg ? data.errmsg : '系统繁忙，请稍后再试');
                    }
