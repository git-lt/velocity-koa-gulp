define(['m_decoration','xss'], function(ModDec){
	...
	CONF = {
		apiAddTpl:'addTemplate.json',
		apiUpdateTpl:'updateTemplate.json',
		apiGetDefConf:'getSupplierFrontConf.json',
		apiInitDefaultTpl:'initSupplierTemplate.json',
		apiGetTplById:'getTemplate.json',//?templateId=
		templateId:'',
		storeId:'',
		templateType:'1',
	...
	page = {
		...
		saveConfEv:function(){
			$('#btnSaveConf').on('click',function(){
				...
				var pms ={
					templateName:filterXSS($.trim($('#tplName').val())),
				...
				pms.templateConfig = JSON.stringify(ModDec.getData());
				_this.uiLoading('sm');
