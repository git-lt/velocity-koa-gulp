			...
			return this.each(function() {
				...
				if (flashInstalled) {
					...
					if (!settings.queueID) {
						...
						if($wrapper.siblings(".uploadify-queue").length == 0){
			...
			this.each(function() {
				...
				if (args[0]) {
					for (var n = 0; n < args.length; n++) {
						...
						swfuploadify.setFileUploadLimit(limit+1);
						$('#' + args[n]).delay(100 + 100 * n).fadeOut(300, function() {
							$(this).remove();
						});
					}
				} else {
				...
				switch(errorCode) {
					case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED:
						if (settings.queueSizeLimit > errorMsg) {
							this.queueData.errorMsg += '最多只能上传' + errorMsg + '个文件';
						} else {
							this.queueData.errorMsg += '文件大小不要超过' + settings.queueSizeLimit + '哦~';
						}
						break;
					case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
						this.queueData.errorMsg += '文件 "' + file.name + '"大小超过' + settings.fileSizeLimit + '';
						break;
					case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
						this.queueData.errorMsg += '文件"' + file.name + '"是空的';
						break;
					case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
						this.queueData.errorMsg += '文件 "' + file.name + '" 应该是' + settings.fileTypeDesc + '类型';
						break;
				}
			}
